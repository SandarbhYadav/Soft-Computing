...
//calculates the rastrigin's function
double rastriginFunction(double *x){
	int i;
	double s1 = 0;
	
	for(i = 0; i < DIM; i++){
		s1 += (x[i]*x[i]-10*cos(C3*x[i]));
	}
...
//		printGeneration(chroms);
		...
		for(i = 0; i < POPULATION_SIZE; i++){
			chroms[i].fitness = rastriginFunction(chroms[i].genes);
			...
			if(fabs(chroms[i].fitness) == 0){
				...
				for(j = 0; j < DIM; j++)
					printf("%lf ", chroms[i].genes[j]);
				printf("- Iterations: %d - Rastrigin's Function = %.8lf\n", counter, chroms[i].fitness);
				free(chroms);
				return 0;
			}
		}
