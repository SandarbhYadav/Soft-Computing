...
//calculates the rosenbrock's function
double rosenbrockFunction(double *x){
	int i;
	double s1 = 0;
	
	for(i = 0; i < DIM-1; i++){
		s1 += (100*(x[i+1]-x[i]*x[i])*(x[i+1]-x[i]*x[i]) + (1-x[i])*(1-x[i]));
	}
...
//		printGeneration(chroms);
		...
		for(i = 0; i < POPULATION_SIZE; i++){
			chroms[i].fitness = rosenbrockFunction(chroms[i].genes);
			...
			if(fabs(chroms[i].fitness) == 0){
				...
				for(j = 0; j < DIM; j++)
					printf("%lf ", chroms[i].genes[j]);
				printf("- Iterations: %d - Rosenbrock's Function = %.8lf\n", counter, chroms[i].fitness);
				free(chroms);
				return 0;
			}
		}
